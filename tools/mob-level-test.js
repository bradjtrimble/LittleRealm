...
const checks=[
  ['mob level engine config + ordinary boss content',!!balance.mobLevels&&Number(mobDefs.snickers?.baseLevel)===8&&mobDefs.snickers?.boss===true],
  ['stable spawn levels',/function mobSpawnLevel/.test(mobs)&&/mobSpawnLevel\(template,tx,ty\)/.test(mobs)],
  ...
  ['goblins above wolves',Number(mobDefs.goblin?.baseLevel)===5&&Number(mobDefs.goblin?.levelMin)===4&&Number(mobDefs.goblin?.levelMax)===6&&Number(mobDefs.wolf?.baseLevel)===4&&Number(mobDefs.wolf?.levelMin)===3&&Number(mobDefs.wolf?.levelMax)===5],
