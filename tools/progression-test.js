...
for(const key of ['slime','goblin','wolf']) if(Number(b.mobs?.[key]?.xpMultiplier)!==1) fail(`${key} should use standard hostile XP`);
for(const key of ['cow','pig','chicken']) if(!(Number(b.mobs?.[key]?.xpMultiplier)<1)) fail(`${key} should use reduced passive XP`);
...
for(const [name,src,pattern] of [
  ...
  ['save migration',save,/xpNext=xpRequiredForLevel\(next\.level\)/],
